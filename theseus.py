...
def buildAllEdges(dimensions, loops=False, string=False, real=True):
    '''
    ...
    real : boolean, optional
        If False, it returns complex weights in polar notation.
...
def conncurence_in_terms_of_weights(avail_states: dict, sys_dict: dict, real=True):
    """
    compute the conncurrence of the state given by the graph according to given avail_states
    ...
    avail_states : dict, optional
        of all graphs that produce the given state(key). The default is None.
    real : TYPE, optional
        complexe or real. The default is True = real.

    Returns
    -------
    Str
        returns concurrence of graph only in terms of graphs weights.

    """

    weights_for_each_ket = []
    corrosponding_kets = len(avail_states) * [0]
    i = 0
    for st, graphes in avail_states.items():
        term_sum = [weightProduct(graph, real) for graph in graphes]
        term_sum = '+'.join(term_sum)
        weights_for_each_ket.append(f'{(term_sum)}')
        corrosponding_kets[i] = st
        i += 1
    # check if no state is produced or states that has less particles than stored in sys_dict
    if len(corrosponding_kets) == 0 or len(corrosponding_kets[0]) != sys_dict['num_particles']:
        print("fail")
        return "0"
    return f'compute_concurrence({corrosponding_kets} ,' + f' {weights_for_each_ket} ,'.replace("'",
                                                                                                "") + f' {sys_dict} )'


# def compute_concurrence_fast(qstate: list, sys_dict: dict):
#     """

#     Parameters
#     ----------
#     state : list
#         state list in state_format (see top)
#     weights : list
#         weights for all kets given in state should have same dimension
#     sys_dict : dict
#         that stores essential infos of quantuum system (see top.get_sysdict).
...
#     def calc_con(mat, par): return np.sqrt(
#         2 * (1 - np.trace((np.linalg.matrix_power(ptrace(mat, par, dimi), 2)))))

#     # np.linalg.norm( -np.array([calc_con(qstate,par[0]) for par in sys_dict['all_biparations'] ]) +
#     return abs(sum([calc_con(qstate, par[0]) for par in sys_dict['all_biparations']]))
#     # np.array([1,1,1,1,np.sqrt(6)/2,np.sqrt(6)/2,np.sqrt(6)/2]) )

def compute_concurrence_fast(state: np.array, sys_dict: dict):
    """
    ...
    sys_dict : dict
        that stores essential infos of quantuum system (see hf.get_sysdict).

    Returns
    -------
    TYPE
        concorrence:  C( |Psi> ) = √( 2 * ( 1 - Trace{ TR_M( <Psi|Psi> )**2 } ) )
        where TR_M is partial trace (in subsystem M)
        and return is sum over all possible bipartion


    """
    qstate = np.array(state, dtype=np.float64)
    dimi = np.array(sys_dict['dimensions'])
    qstate *= 1 / (np.linalg.norm(qstate))

    def calc_con(mat, par):
        red = ptrace(mat, par, dimi, False)
        return (2 * (1 - min(1, np.einsum('ij,ji', red, red)))) ** 0.5

    return sum([calc_con(qstate, par[0]) for par in sys_dict['all_bipartitions']])


def conncurence_in_terms_of_weights_fast(avail_states: dict, sys_dict: dict):
    """
    compute the conncurrence of the state given by the graph according to given avail_states
    ...
    for idx, state in enumerate(sys_dict["all_states"]):
        try:
            term_sum = [weightProduct(graph, sys_dict['real'])
    ...
    return f'compute_concurrence_fast({state_vector} ,'.replace("'", "") + f' {sys_dict} )'
...
def compute_entangelment(qstate: np.array, sys_dict: dict) -> float:
    ...
    def calc_con(mat, par):
        red = ptrace(mat, par, dimi, False)
        d_sub = red.shape[0]
        return np.linalg.norm(red - np.identity(d_sub) * 1 / d_sub)

    loss_vec = [calc_con(qstate, par[0]) for par in sys_dict['bipar_for_opti']]
    lenght = len(loss_vec)
    mean = sum(loss_vec) / lenght
    if sys_dict['var_factor'] == 0:  # no need to compute variance if factor = 0
        return mean
    else:
        var = sum([(x - mean) ** 2 for x in loss_vec]) / (lenght)
        return mean + sys_dict['var_factor'] * var


def entangelment_fast(avail_states: dict, sys_dict: dict):
    """
    compute the entanglement according to compute_entangelment()
    ...
    return f'compute_entangelment(np.array({state_vector} ),'.replace("'", "") \
           + f' {sys_dict} )'

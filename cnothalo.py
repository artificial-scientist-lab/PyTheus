...
locdim = [2]*2 + [3] + [4] + [1]*(pdv[2] - pdv[0])
edge_list = th.buildAllEdges(locdim)
# remove all connections that will be fixed later, this way they cant be removed by topopt
edge_list = top.removeConnections(edge_list, [[0,1],[0,2],[0,3],[1,2],[1,3],[2,3],[4,5],[6,7],[8,9]])
# fix one pm for ancillas and a K4 graph in the  to exclude unwanted hyperedges
fixededges = [(4,5,0,0),(6,7,0,0),(8,9,0,0),(0,1,4,4),(0,2,4,4),(0,3,4,4),(1,2,4,4),(1,3,4,4),(2,3,4,4)]

#define CNOT action as state creation
state = top.makeState("0000+0011+0022+1101+1112+1123")
coeff = None
...
if showgraphs: graph_init = gp.graphPlot(edge_list, scaled_weights=True, show=True, max_thickness=10)
...
cr, cr_string = top.makeLossString(state, edge_list, coeff=coeff, real=real, fixededges=fixededges)
fid, fid_string = top.makeLossString(state, edge_list, coeff=coeff, mode="fid", real=real, fixededges=fixededges)
...
while samples > 0:
    ...
    while not condition:  # keep going until truncated optimization is good
        ...
        if numtrunc == 0:
            ...
            cr_new, _ = top.makeLossString(state, edge_list_new, coeff=coeff, real=real, fixededges=fixededges)
            fid_new, _ = top.makeLossString(state, edge_list_new, coeff=coeff, mode="fid", real=real, fixededges=fixededges)
...
# -
if showgraphs: graph_sol = gp.graphPlot(edge_list_cur, scaled_weights=True, show=True, max_thickness=10)
